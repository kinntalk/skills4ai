...
### LaTeX Engine (Optional for Pandoc)
- **macOS**: `brew install --cask mactex`
- **Windows**: Install MiKTeX or TeX Live

### Alternative: Node.js (md-to-pdf)
If Pandoc/LaTeX is not available, you can use the Node.js fallback:
```bash
...
- **Python 3.6+**
- **Pandoc** (Recommended for high quality)
- **LaTeX** (Optional, for Pandoc pdf-engine)
- **Node.js** (Optional, fallback renderer)

## Internationalization (CJK Support)

To generate PDFs containing Chinese, Japanese, or Korean characters, the `generate_pdf.py` script automatically detects the operating system and attempts to use an appropriate system font:

- **Windows**: Microsoft YaHei (微软雅黑)
- **macOS**: PingFang SC (苹方)
- **Linux**: Noto Sans CJK SC

**Note**: This requires the `xelatex` engine (part of MiKTeX/TeX Live) to be installed. If characters are missing in the output PDF, ensure the relevant fonts are installed on your system.
